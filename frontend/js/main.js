$(document).ready(function() {
  ...
  function showGoodsNumber(element) {
    ...
    if ( $(element).is(':checked') ){
      let yPos = $(element).parent().offset().top - $(element).closest('.filter-aside__section').offset().top - 2
      let numberElement = $("<div class='filter-aside__number'>Показать 67 товаров</div>").css('top', yPos + 'px');
      $(element).parent().append(numberElement);
      setTimeout(function(){
        let numberElementWidth = numberElement.width();
